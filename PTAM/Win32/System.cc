  ...
  : mGLWindow(mVideoSource.Size(), "PTAM")
{
  ...
  mimFrameBW.resize(mVideoSource.Size());//黑白
  mimFrameRGB.resize(mVideoSource.Size());//彩色
  ...
  // 创建地图Map 地图创建管理器MapMaker 跟踪器Tracker 增强现实AR驱动ARDriver 地图显示MapViewer 
  mpMap = new Map;
  ...
  while(!mbDone)
    {
      ...
      if(bFirstFrame)
	{
	  mpARDriver->Init();//初始化，主要用于生成纹理标识及FrameBuffer
	  bFirstFrame = false;
	}
      //设置窗口属性
