...
bool Bundle::Do_LM_Step(bool *pbAbortSignal)
{
  ...
  while(dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations && !*pbAbortSignal)
      ...
      for(unsigned int i=0; i<mvPoints.size(); i++)
	{
	  ...
	  //dqc modified
#ifdef WIN32 	  if(_isnan(vMapUpdates.slice(i * 3, 3) * vMapUpdates.slice(i * 3, 3)))
#else 	  if(isnan(vMapUpdates.slice(i * 3, 3) * vMapUpdates.slice(i * 3, 3)))
#endif    
	    {
	      cerr << "NANNERY! " << endl;
